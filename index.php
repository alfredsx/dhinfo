...
if ($info->getVar('title_sicht') == 1) {
	$xoopsTpl->assign('title', $info->getVar('title'));
}
...
if (in_array(constant('_CON_' . $lang_name . '_CANUPDATEALL'),$show_info_perm)) {
	...
	if ($xoopsUser) {
		if ($info->getVar('owner') == $xoopsUser->getVar('uid') || in_array(XOOPS_GROUP_ADMIN,$infothisgroups)) {
			$canedit = true;
		}
