...
$tueber = ($id > 0) ? _AM_INFO_ADMENU1 : _AM_INFO_ADDCONTENT;
...
if ( (in_array(_CON_INFO_ALLCANUPDATE_CAT,$show_info_perm) && $id == 0) || (in_array(_CON_INFO_CANUPDATE_CAT,$show_info_perm) && $id > 0) || $mod_isAdmin) {
  $block_select = new XoopsFormSelect(_AM_INFO_HOMEPAGE, "cat",$content->getVar('cat'));
...
$form->addElement(new XoopsFormText(_AM_INFO_LINKNAME, "title", 80, 255,$content->getVar('title')),true); 
$form->addElement(new XoopsFormText(_MI_INFO_TOOLTIP, "ttip", 80, 255,$content->getVar('tooltip')),false);

if ( in_array($content->getVar('link'),array(0,1,2,4,5)) ) {
	$title_sicht = new XoopsFormCheckBox(_AM_INFO_TITLESICHT, 'title_sicht',$content->getVar('title_sicht'));
	$title_sicht->addOption(1, _YES);
	$form->addElement($title_sicht);
  $footer_sicht = new XoopsFormCheckBox(_AM_INFO_FOOTERSICHT, 'footer_sicht',$content->getVar('footer_sicht'));
...
if ( (in_array(_CON_INFO_ALLCANUPDATE_POSITION,$show_info_perm) && $id == 0) || (in_array(_CON_INFO_CANUPDATE_POSITION,$show_info_perm) && $id > 0) || $mod_isAdmin) { 
  $categoria_select = new XoopsFormSelect(_AM_INFO_POSITION, "parent_id",$content->getVar('parent_id'));
...
$form->addElement(new XoopsFormText(_AM_INFO_LINKID, "blockid", 5, 5,$content->getVar('blockid')),false); 
...
if ( (in_array(_CON_INFO_ALLCANUPDATE_SITEART,$show_info_perm) && $id == 0) || (in_array(_CON_INFO_CANUPDATE_SITEART,$show_info_perm) && $id > 0) || $mod_isAdmin) {
  $url_art = new XoopsFormSelect(_AM_INFO_URLART, "link", $content->getVar('link'));
  $url_art->addOption(0, _AM_INFO_URL_NORMAL);
  $url_art->addOption(6, _AM_INFO_URL_PHP);
  $url_art->addOption(3, _AM_INFO_URL_KATEGORIE);
  $url_art->addOption(2, _AM_INFO_URL_EXTLINK);
  $url_art->addOption(1, _AM_INFO_URL_INTLINK);
  $url_art->addOption(5, _AM_INFO_URL_IFRAME);
  $url_art->addOption(4, _AM_INFO_URL_INTDATEI);	
...
if (in_array($content->getVar('link'),array(1,2,4,5))) { 
    switch ($content->getVar('link')) {
        case 2:
            $form->addElement(new XoopsFormText(_AM_INFO_URL_EXTERN, "address", 80, 255,$content->getVar('address')),true); 
            ...
            break;
        case 1:
            $form->addElement(new XoopsFormText(_AM_INFO_URL_INTERN, "address", 80, 255,$content->getVar('address')),true); 
            $iframe = unserialize($content->getVar('frame'));
            $form->addElement(new XoopsFormHidden("height", $iframe['height'])); 
            $form->addElement(new XoopsFormHidden("border", $iframe['border']));
            $form->addElement(new XoopsFormHidden("width", $iframe['width']));
            ...
            break;
        case 4:
            $form->addElement(new XoopsFormText(_AM_INFO_URL_DATEI, "address", 80, 255,$content->getVar('address')),true); 
            $iframe = unserialize($content->getVar('frame'));
            $form->addElement(new XoopsFormText(_AM_INFO_URL_FRAME_HEIGHT, "height", 5, 5,$iframe['height']),true); 
            $form->addElement(new XoopsFormText(_AM_INFO_URL_FRAME_WIDTH, "width", 5, 5,$iframe['width']),false);
            $form->addElement(new XoopsFormHidden("border", $iframe['border']));
            $form->addElement(new XoopsFormHidden("align", $iframe['align']));
            break;
        case 5:
            $iframe = unserialize($content->getVar('frame'));
            $form->addElement(new XoopsFormText(_AM_INFO_URL_FRAME, "address", 80, 255,$content->getVar('address')),true); 
            $form->addElement(new XoopsFormText(_AM_INFO_URL_FRAME_HEIGHT, "height", 5, 5,$iframe['height']),true); 
            $form->addElement(new XoopsFormText(_AM_INFO_URL_FRAME_BORDER, "border", 5, 5,$iframe['border']),true);
            $form->addElement(new XoopsFormText(_AM_INFO_URL_FRAME_WIDTH, "width", 5, 5,$iframe['width']),false);
            $frame_align=new XoopsFormSelect(_AM_INFO_URL_FRAME_ALIGN, "align", $iframe['align']);
...
if ($content->getVar('link') == 1 || $content->getVar('link') == 2) {
	$menu_selfbox = new XoopsFormCheckBox(_AM_INFO_SELF, 'self',$content->getVar('self'));
...
if (intval($content->getVar('link')) == 3) {
	$menu_clickbox = new XoopsFormCheckBox(_AM_INFO_CLICK, 'click',$content->getVar('click'));
...
$visible_checkbox = new XoopsFormCheckBox(_AM_INFO_VISIBLE, 'visible',$content->getVar('visible'));
...
if (intval($content->getVar('link')) == 3) {
  ...
  $menu_checkbox = new XoopsFormCheckBox(_AM_INFO_SUBMENU, 'submenu',$content->getVar('submenu'));
...
if ( (in_array(_CON_INFO_ALLCANUPDATE_GROUPS,$show_info_perm) && $id == 0) || (in_array(_CON_INFO_CANUPDATE_GROUPS,$show_info_perm) && $id > 0) || $mod_isAdmin) {
  $groups = new XoopsFormSelectGroup(_AM_INFO_VISIBLE_GROUP, 'visible_group', true, $sgroup, 5,true);
...
$form->addElement(new XoopsFormRadioYN(_AM_INFO_VISIBLE_LEFTBLOCK, 'bl_left',$content->getVar('bl_left'), $yes=_YES, $no=_NO));
$form->addElement(new XoopsFormRadioYN(_AM_INFO_VISIBLE_RIGHTBLOCK, 'bl_right',$content->getVar('bl_right'), $yes=_YES, $no=_NO));
...
if (intval($content->getVar('link')) == 0 || intval($content->getVar('link')) == 4 || intval($content->getVar('link')) == 6) {
  ...
  if ($xoopsModuleConfig['com_rule'] && $xoopsModuleConfig['com_rule'] > 0) {
    ...
    $comments_checkbox->addOption(1, _AM_INFO_DISABLECOM);
...
$form->addElement(new XoopsFormLabel(_AM_INFO_OWNER, $oUser));	
$last_editor = $eUser;
$euser = (is_object($xoopsUser)) ? $xoopsUser->uid() : 0;
if ($id == 0) {
	$form->addElement(new XoopsFormLabel(_AM_INFO_LAST_EDITED,_AM_INFO_NEWADDSITE));
} else {
  ...
  $form->addElement(new XoopsFormLabel(_AM_INFO_LAST_EDITED, sprintf(_AM_INFO_LAST_EDITEDTEXT,$last_editor,$last_time)));
}
